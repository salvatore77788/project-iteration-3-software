...
public class ReturnChangeSoftware {
	...
	public ReturnChangeSoftware(SelfCheckoutStation station) throws SimulationException, OverloadException {
		// connect the checkout station from the SelfCheckoutStationSoftware to this class
		this.scs = station;
		
		// initialize the banknoteDenominations since the original copy provides a hashcode instead of an array. 
		for(int i : this.scs.banknoteDenominations) {
			banknoteDenominations.add(Integer.valueOf(i));
		}
		
		// for each type of denomination, load the denomination into their respective dispenser to the maximum capacity. 
		for(int i : this.scs.banknoteDenominations) {
			int counter = 0;
	 ...
	 * 	 @return void
	 * */
	public void refillCoinDispenser(Coin... coins) throws SimulationException, OverloadException {
		for (Coin c: coins) {
			if (c == null) {
				throw new SimulationException("This coin is null.");
			}
	 ...
	 * 	 @return void
	 * */
	public void refillBanknoteDispenser(Banknote ...banknotes) throws SimulationException, OverloadException {
